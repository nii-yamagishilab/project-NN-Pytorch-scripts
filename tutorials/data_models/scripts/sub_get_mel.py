...
def get_melsp(input_file, fftl=1024, fl=400, fs=80):
    """get_melsp(input_file, fftl=1024, fl=400, fs=80)
    
    input:
    -----
      input_file: string, path to the input waveform file
      fftl: int, number of FFT points, default 1024
      fl: int, frame length (in number of waveform points), default 400
      fs: int, frame shift (in number of waveform points), default 80

    output
    ------
      output: np.array, Mel-spectrogram [number_of_frames, frame_dim]

    Note: fl = 400  -> 25 ms frame length when the waveform is 16000 Hz
             400 = 16 * 25
    """
    # input waveform
    if os.path.isfile(input_file):
        sp = SpeechProcessing(fftl=fftl, fl=fl, fs=fs)
        with wave.Wave_read(input_file) as f:
            T = np.frombuffer(f.readframes(f.getnframes()), np.int16).astype(np.float32)
            X = sp.analyze(T)
        return np.array(X, dtype=np.float32)
    else:
        print("cannot find {:s}".format(input_file))
        return None
