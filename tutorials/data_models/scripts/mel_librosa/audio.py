...
def get_melsp_librosa(input_file):
    """output = get_melsp_librosa

    input
    -----
      input_file: str, string to the waveform file
     
    output
    ------
      output: np.array, output melspec, [num_frame, mel_dim]

    Note that the configuration is specified in hparam.py
    
    """
    hparams_ins = Hparams_class()
    audio = Audio(hparams_ins)
    wav = audio.load_wav(input_file)
