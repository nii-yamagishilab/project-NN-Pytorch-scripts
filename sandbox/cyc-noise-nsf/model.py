...
class SignalsConv1d(torch_nn.Module):
    ...
    def forward(self, signal, system_ir):
        ...
        signal:    (batchsize=1, length1, dim)
        system_ir: (length2, dim) 
        
        output:    (batchsize=1, length1, dim)
        """ 
...
class SineGen(torch_nn.Module):
    ...
    def _f02sine(self, f0_values):
        ...
        if not self.flag_for_pulse:
            ...
            # remove the accumulation across different voiced segments
            i_phase = torch.cumsum(rad_values - i_phase * u_loc, dim=1)
            # get the sines
...
class Model(torch_nn.Module):
    ...
    def __init__(self, in_dim, out_dim, args, mean_std=None):
        ...
        # one additional module to produce sine mask
...
class Loss():
    ...
    def _mask_stft(self, mask_signal, fft_p, frame_shift, frame_len):
        ...
        # ignores unvoiced frames, based on power
