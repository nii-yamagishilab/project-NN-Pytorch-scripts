#!/usr/bin/env python
"""
implementation of iLPCNet. 

Functions here are from 05-nn-vocoders/ilpcnet. 
For convenience, they are moved to this folder. 

For usage, please check 05-nn-vocoders/ilpcnet.

Ref:
Min-Jae Hwang, Eunwoo Song, Ryuichi Yamamoto, Frank Soong, and Hong-Goo Kang. 
2020. Improving LPCNet-based text-to-speech with linear prediction-structured 
mixture density network. In Proc. ICASSP, pages 7219–7223. IEEE. 
"""
...
def LPCanalysis_api(wav, frame_length, frame_shift, lpc_order):
...
class LPCNetV1(torch_nn.Module):
    """LPCNetV1(framelen, frameshift, lpc_order, cond_dim, flag_stage2)
    
    input_acoustic_feature -> | self.m_cond_net | -> reflection coef & gain
       -> | rc2lpc (deterministic) | -> LP coef -> | lp (deterministic) | 
       -> LP residual -> training the LPC network
    ...
    ----
      ...
      flag_stage2: bool, whether fix the condition network or not
      
    In training, we can do a two-staged training: 
    1. train the condition network self.m_cond_net (flag_stage2 = False)
    2. fix the condition network, train the rest of iLPCNet (flag_stage2=True)
    ...
    def __init__(self, framelen, frameshift, lpc_order, cond_dim, 
        ...
        # fix
        self.flag_stage2 = flag_stage2
        if self.flag_stage2:
            print("Info iLPCNet: training stage 2")
        else:
            print("Info iLPCNet: training stage 1")
    ...
    def forward(self, cond_feat, cond_feat_normed, f0,
        ...
        We assume that conf_feat[..., -1] encodes the F0 value
        """
        ...
        cond_len = np.min([rc_coef.shape[1], cond_feat.shape[1], f0.shape[1]])
        if self.flag_stage2:
            self.m_cond_net.eval()
            with torch.no_grad():
        ...
        with torch.no_grad():
            ...
            pitch_quantized = self._convert_pitch(f0[..., 0])
        ...
        lpccond_feat = self.m_net_framerate(
            torch.cat([cond_feat_normed[:, :cond_len], pitch_emb[:, :cond_len]],
                      dim=-1))
    ...
    def inference(self, cond_feat, cond_feat_normed, f0):
        ...
        #  loop
        for idx in tqdm(range(0, wavlen)):
            #if idx % 1000 == 0:
