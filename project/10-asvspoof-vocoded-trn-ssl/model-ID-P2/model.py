...
class FrontEnd(torch_nn.Module):
    ...
    def __init__(self, mpath, ssl_out_dim, 
        ...
        self.ssl_model2 = SSLModel('', ssl_out_dim, ssl_layer_indices)
    ...
    def duplicate(self, feat, num):
        ...
        if self.flag_frontend_hid:
            return [feat[0].repeat(num, 1, 1),
                    ...
                    [feat[-1][0].repeat(num, 1, 1),
                     feat[-1][1].repeat(num, 1, 1),
                     feat[-1][2].repeat(num, 1, 1, 1),
                     feat[-1][3].repeat(num, 1, 1, 1)]]
        else:
    ...
    def __forward_multi_view(self, x_tuple, fileinfo):
        ...
        if self.m_feat or self.m_rank:
            ...
            if type(feat_vec) is tuple:
                feat_vec = [feat_vec[0], feat_vec[-1][0], feat_vec[-1][1]]
            ...
            for bat_idx in range(bat_siz):
                ...
                for tmp_vec in feat_vec:
                    feat_vec_ = tmp_vec.view(
                        bat_siz, num_sys, num_view, -1, tmp_vec.shape[-1])
                    loss_cr_1 += self.m_feat / bat_siz * self.m_cr_loss(
                        feat_vec_[bat_idx], feat_class[bat_idx]) / len(feat_vec)
