...
from tools.text_tools import toolkit_all
from tools.text_tools import toolkit_en
...
def text2code(text, flag_lang='EN'):
    ...
    if flag_lang == 'EN':
        ...
        print("Error: text2code cannot handle {:s}".format(flag_lang))
        sys.exit(1)
    
    # convert to numpy format
    #code_seq = np.array(code_seq, dtype=np.float32)
    code_seq = np.array(code_seq, dtype=np.int64)
...
def code2text(codes, flag_lang='EN'):
    ...
    if flag_lang == 'EN':
        ...
        print("Error: code2text cannot handle {:s}".format(flag_lang))
        sys.exit(1)
    return output_text
...
def symbol_num(flag_lang='EN'):
    ...
    if flag_lang == 'EN':
        ...
        f_die("Error: symbol_num cannot handle {:s}".format(flag_lang))
    return 0
...
def eos_index(flag_lang='EN'):
    ...
    if flag_lang == 'EN':
        ...
        f_die("Error: eos_index cannot handle {:s}".format(flag_lang))
    return 0
...
def g2p2code(text, flag_lang='EN'):
    ...
    if len(text_trunks) > 1 or text_trunks[0][0] == toolkit_all._curly_symbol:
        # unsupporte languages
        f_die("Error: g2p2code don't process: {:s}".format(text))
    ...
    if flag_lang == 'EN':
        ...
        try:
            ...
            f_die("G2p tool not found. Please install g2p_en")
        ...
        # unsupporte languages
        f_die("Error: text2code cannot handle {:s}".format(flag_lang))

    return code_seq

def rawtextloader(file_path):
    # load lines and chop '\n', join into one line
    text_buffer = [string_chop(x) for x in open(file_path, 'r')]
    text_buffer = ' '.join(text_buffer)
    return text_buffer

def _text2code(text_buffer, flag_lang, g2p_tool):
...
def textloader(file_path, flag_lang='EN', g2p_tool=None):
    ...
    return _text2code(text_buffer, flag_lang, g2p_tool)
